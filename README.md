...
- Docs live under `docs/` (usage, architecture, cost plan, security, report template).
 ...
 - Budget state lives in `BUDGET_DIR/budget.json` (default `.budget/budget.json`); the runner prints workload and a budget‑aware `SUBSET` suggestion. See `docs/USAGE.md#budget-state-and-overrides`.
